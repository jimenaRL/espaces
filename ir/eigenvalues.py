...
def h2e1(F=[0.1], c=3.4e2, j_max=1, kind=0, **unusedkwargs):
    ...
    def hyperbolic(j_max=1,kind=0):
        ...
        kind_index = { 0 :'eig-maxsymm-24.txt',
                       1 :'eig-maxsymm-48.txt',
                       2 : 'eig-octagon.txt',
                       3 : 'eig-pol-1-0-500.dat',
                     }

        path = os.path.join(ESPACES_PROJECT,'dev','ir','eigenvalues',kind_index[kind])
...
def eigenvalues(params):
    ...
    eigenvalues_ = globals()[params['space']](**_check_params(params))
    return eigenvalues_ 