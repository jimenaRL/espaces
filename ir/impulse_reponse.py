...
def _check_params(params):
    ...
    params.update({
        k: v if k not in params
        else params[k] for k, v in default_params.items()})
...
def get_ir(ir_params):
    """
        Computes the impulse reponse (ir)  at x=0 associated with
        the Green function of the wave equation in a manifold M
        defined by its eigen-values.

        Return an instance of the class Signal containing the Green
        function at given sampling rate.
